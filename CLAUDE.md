...
## Trocar jogadores — investigação de dificuldade (concluída, decisão: FINAL DA FILA / pós-SPAC)
DIFICULDADE: MÉDIA — não por complexidade algorítmica, mas por número de pontos tocados. Maior parte é REUSO.
FÁCIL (já existe): restartMatch (l.1683-1691, zera+rebuildEngine) + startServerDraw (l.1554-1585, sorteio animado) prontos; propagação por set_config + kind:reset (receptor já trata isReset l.976-980) funciona; previews/foto seguem os ids sozinhos (previewDoSlot deriva de playerIds); ConfirmModal existe.
DIFÍCIL (novo): o MODO TROCA na UI (terceiro estado do slot — de <Input> editável para card selecionável), estado modoTroca+slotSelecionado, restrição adversário (2º toque só vale se time(a)≠time(b)), o pulso ao ativar (re-disparo com key/remoção — o alerta de sempre), trocarSlots(a,b) (permuta players E playerIds num patch — o saveAllNames atual só manda players, não serve).
O RISCO DO SAQUE FOI ELIMINADO pela decisão de produto (troca = jogo novo): re-sortear em vez de permutar; restartMatch zera o motor, chooseServer só age no pré-jogo (guarda actionsRef vazio). Resíduo: zerar gameConfig.initialServer junto da troca (uma linha) para não sobrar índice velho na janela até o sorteio.
CONFIRM obrigatório (reset destrutivo): reusar ConfirmModal, texto "Trocar os jogadores começa uma partida nova: o placar zera e o saque é sorteado de novo."
PERGUNTA BLOQUEANTE ao Supabase (antes de implementar): o prosrc de apply_live_match_action — o kind:'reset' limpa só state.actions ou o state INTEIRO? Se for o inteiro, apagaria players/playerIds da troca — a ordem das chamadas muda (troca depois do reset, ou reset preserva players/playerIds).
